...
int getMinimumPenalty(std::string x, std::string y, int pxy, int pgap, int *xans, int *yans, int n_threads, int rank);
...
inline RESULT_t do_job(std::string x, std::string y, int job_id, int misMatchPenalty, int gapPenalty, int n_threads, int rank);
	...
	int *penalties)
{
    ...
    int n_threads1 = 1;
    int n_threads2 = CPU_PER_TASK - n_threads1;
    ...
    {
        if (omp_get_thread_num() == 0) // the leader task
        {
        ...
        else // woker task
        {
            ...
            while (STOP != STOP_SYMBOL){
                uint64_t checkpoint1 = GetTimeStamp();
                RESULT_t result = do_job(genes[my_job.i], genes[my_job.j], my_job.id, pxy, pgap, CPU_PER_TASK-1, root);
                uint64_t checkpoint2 = GetTimeStamp();
                computation_time += checkpoint2 - checkpoint1;
            ...
            cout << "rank-" << root << " takes " << end_ - start_ << "us for matrix "<< endl;
            cout << "rank-" << root << " computes " << computation_time << "us for matrix "<< endl;
...
void do_MPI_task(int rank)
{
    ...
    while (STOP != STOP_SYMBOL){
        uint64_t checkpoint1 = GetTimeStamp();
        RESULT_t result = do_job(genes[my_job.i], genes[my_job.j], my_job.id, misMatchPenalty, gapPenalty, CPU_PER_TASK, rank);
        uint64_t checkpoint2 = GetTimeStamp();
        computation_time += checkpoint2 - checkpoint1;
    ...
    uint64_t end_ = GetTimeStamp();
    cout << "rank-" << rank << " takes " << end_ - start_ << "us for matrix "<< endl;
    cout << "rank-" << rank << " computes " << computation_time << "us for matrix "<< endl;
}

// function to find out the minimum penalty
// return the minimum penalty and put the aligned sequences in xans and yans
inline int getMinimumPenalty(std::string x, std::string y, int pxy, int pgap, int *xans, int *yans, int n_threads, int rank)
{
    ...
    #pragma omp parallel
    {
        if (omp_get_thread_num() == 0){
            cout << "rank-" << rank << ": get " << omp_get_num_threads() << " threads" << endl; 
        }
    }
            ...
            for (int ii = tile_row_start; ii < tile_row_end; ++ii) {
                for (int jj = tile_col_start; jj < tile_col_end; ++jj) {
                    if (x[ii - 1] == y[jj - 1]) {
                        ...
                        dp[ii][jj] = min(min(dp[ii - 1][jj - 1] + pxy,
                                          dp[ii - 1][jj] + pgap),
                                          dp[ii][jj - 1] + pgap);
                    }
                }
...
inline RESULT_t do_job(std::string gene1, std::string gene2, int job_id, int misMatchPenalty, int gapPenalty, int n_threads, int rank){
    
    int m = gene1.length(); // length of gene1
    int n = gene2.length(); // length of gene2
    int l = m+n;
    int xans[l+1], yans[l+1];
    int penalty = getMinimumPenalty(gene1,gene2,misMatchPenalty,gapPenalty,xans,yans,n_threads, rank);
