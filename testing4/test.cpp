    ...
    for(int i=1;i<k;i++){
		for(int j=0;j<i;j++){
            if (task_id >= my_tasks_start && task_id < my_tasks_end) {
                ...
                // calculate max strng length
                if (i_max_length < genes_length[i]) {
                    i_max_length = genes_length[i];
                }
                if (j_max_length < genes_length[j]) {
                    j_max_length = genes_length[j];
                }
    ...
    for(int i=1;i<k;i++){
		for(int j=0;j<i;j++){
            if (task_id >= my_tasks_start && task_id < my_tasks_end) {
                tasks.push_back({ i, j, task_id });
                // calculate max strng length
                if (i_max_length < local_genes_len[i]) {
                    i_max_length = local_genes_len[i];
                }
                if (j_max_length < local_genes_len[j]) {
                    j_max_length = local_genes_len[j];
                }
            } else if (task_id >= my_tasks_end) {
                break;
            }
