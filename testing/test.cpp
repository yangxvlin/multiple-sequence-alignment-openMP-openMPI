...
inline int getMinimumPenalty2(std::string x, std::string y, int pxy, int pgap,
                      int *xans, int *yans, int m, int n) {
...
//	memset (dp[0], 0, size);
    ...
    // Tile parallel
    int n_parallel = n_threads + ((int) ceil(1.0 * n_threads / 3));
    // calculate tile size
